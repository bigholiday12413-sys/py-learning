...
ブラウザ自動化ライブラリ **Playwright** を使って、動的なWebページからデータを取得する方法を学ぶ。

最近のWebサイトの多くは、ページを開いた後にブラウザ内のプログラム（JavaScript）がデータを取得して画面を組み立てる「動的な」作りになっている。Lv04（requests + BeautifulSoup）ではこうしたサイトのデータは取得できなかったが、Playwright なら本物のブラウザを動かすため対応できる。
...
|------|----------------------|------------------|
| 動的ページの描画 | 不可（HTMLのみ取得） | 可能（実ブラウザで実行） |
| SPA（後からデータを読み込むサイト）対応 | 不可 | 可能 |
| ボタンクリック | 不可 | 可能 |
| スクリーンショット | 不可 | 可能 |
