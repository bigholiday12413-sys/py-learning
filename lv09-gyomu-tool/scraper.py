...
ポイント:
  セレクタを config.json に外出ししているので、
  対象サイトの構造が変わってもコードを直さず設定変更だけで対応できる。
"""
...
class PageScraper:
    ...
    def scrape(self, start_url: str) -> list[dict]:
        ...
        「データ抽出 → 次ページがあれば遷移」を while ループで繰り返す。
    ...
    def _extract_page_data(self) -> list[dict]:
        ...
        for book in books:
            try:
                # --- 各書籍の情報を抽出 ---
                # title 属性がなければ表示テキストを使う（or によるフォールバック）
    ...
    def _has_next_page(self) -> bool:
        ...
        locator.count() が 0 なら「次へ」リンクがない = 最終ページ。
        """
